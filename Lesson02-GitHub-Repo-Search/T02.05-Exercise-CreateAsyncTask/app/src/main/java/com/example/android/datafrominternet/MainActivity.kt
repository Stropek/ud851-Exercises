...
class MainActivity : AppCompatActivity() {
     ...
     * (using [NetworkUtils]) for the github repository you'd like to find, displays
    ...
    inner class GithubQueryTask : AsyncTask<URL, Void, String>()
    {
        override fun doInBackground(vararg urls: URL): String {
            var saerchUrl = urls[0]
            var githubySearchResults : String? = null

            try {
                githubySearchResults = NetworkUtils.getResponseFromHttpUrl(saerchUrl)
            } catch (e: IOException) {
                e.printStackTrace()
            }

            return githubySearchResults.toString()
        }
