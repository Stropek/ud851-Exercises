...
class SettingsFragment : PreferenceFragmentCompat(), OnSharedPreferenceChangeListener {
    ...
    override fun onCreatePreferences(bundle: Bundle?, s: String?) {
        ...
        for(preferenceId in 0..(preferenceScreen.preferenceCount - 1)) {
            val pref = preferenceScreen.getPreference(preferenceId)
            if (pref !is CheckBoxPreference) {
                setPreferenceSummary(pref, preferenceScreen.sharedPreferences.getString(pref.key, ""))
            }
        }
    }
